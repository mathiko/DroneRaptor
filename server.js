...
app.post('/create-location-file', (req, res) => {
    const { filename, lat, lng } = req.body;
    ...
    createLocationFileInTerminal(filename, lat, lng, res);
});

app.post('/create-motion-location-file', (req, res) => {
    const { filename, lat, lng, lat2, lng2 } = req.body;
    // For now, assuming filename is the name of the location
    const newLocation = { name: filename, path: `/home/${username}/Desktop/gps-sdr-sim-master/${filename}.bin` };
    locationParameters.push(newLocation);
    // Run the command to create the location file
    createMotionLocationFileInTerminal(filename, lat, lng, lat2, lng2, res);
});
...
function createLocationFileInTerminal(filename, lat, lng, res) {
    const command = `/home/${username}/Desktop/gps-sdr-sim-master/./gps-sdr-sim -e "/home/${username}/Desktop/gps-sdr-sim-master/brdc1010.24n" -l ${lat},${lng},100 -b 8 -d 25 -o "/home/${username}/Desktop/gps-sdr-sim-master/${filename}.bin"`;
...
function createMotionLocationFileInTerminal(filename, lat, lng, lat2, lng2, res) {
    const command1 = `/home/${username}/Desktop/gps-sdr-sim-master/umc.exe 100 ${lat} ${lng} ${lat2} ${lng2}`;
    ...
    const command2 = `/home/${username}/Desktop/gps-sdr-sim-gui-master/./gps-sdr-sim -x "/home/${username}/Desktop/Nettside/Trajectory.csv" -b 8 -d 100 -e "/home/${username}/Desktop/gps-sdr-sim-master/brdc1070.24n" -o "/home/${username}/Desktop/gps-sdr-sim-master/${filename}.bin"`;
