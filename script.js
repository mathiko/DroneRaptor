...
//This function hides the errormessage again, and therefore changes the displaytype from "block" to "none", which effectively hides the content of the element.
...
function spoofFile() {
    ...
    .then(response => { //.then happens after the fetch, and takes care of the response. If the respons.ok is true, the HTTP-request was successful. If it was not ok, the request was unsuccessful.
        if (response.ok) {
            ...
            hideErrorMessage(); //Hides errormessage if there ever was one.
            displaySuccessMessage("Spoofing started successfully."); //Displays successmessage on the HTML-page.
        } 
...
function jamFile() {
    ...
    .then(response => { //.then happens after the fetch, and takes care of the response. If the respons.ok is true, the HTTP-request was successful. If it was not ok, the request was unsuccessful.
        if (response.ok) {
            console.log("Jamming started successfully."); //Logs in the terminal that spoofing is started.
            hideErrorMessage(); //Hides errormessage if there ever was one.
            displaySuccessMessage("Jamming started successfully."); //Displays successmessage on the HTML-page.
        } 
...
function createLocationFile() {
    ...
    if (!checkbox.checked) {
        ...
        .then(response => {
            if (response.ok) {
                ...
                hideErrorMessage();
                displaySuccessMessage("Location file created successfully.")
            } 
    ...
    else { //Fetch connects to the "create-motion-location-file" endpoint made in server.js, and then POST's lat, lng, lat2, lng2 and dur to it as JSON format.
        ...
        .then(response => {
            if (response.ok) {
                console.log('Motion location-file created successfully');
                addLocationToSelect(filename);
                hideErrorMessage();
                displaySuccessMessage("Motion location-file created successfully.")
            } 
